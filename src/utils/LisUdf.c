    ...
    while (getline(&Unidad, &TamUnidad, FpLis) != -1)
    {
        ...
        *LisUdf = tmplis;
        ...
        if (TamLis == SIZE_MAX - 1)
        {
            ...
            fprintf(stderr,
                    "La lista de fonemas ha alcanzado su tamanho maximo\n");
            LiberaMatStr(*LisUdf);
            fclose(FpLis);
            free(Unidad);
            return LIS_UDF_ERROR;
...
size_t MeteLisUdf(char *Unidad, size_t * TamLis, char ***LisUdf)
{
    ...
    if (*LisUdf == NULL)
    {
        ...
        return 0;
              ...
              SeekLisUdf(Unidad, *TamLis, *LisUdf)) == LIS_UDF_UNKNOWN)
    {
        Indice = *TamLis;
        if (*TamLis == SIZE_MAX - 1)
        {
            fprintf(stderr,
                    "Error: La lista ha alcanzado su tamanho maximo\n");
            LiberaMatStr(*LisUdf);
            *LisUdf = NULL;
            return LIS_UDF_ERROR;
        }
