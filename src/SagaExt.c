...

{
	...
	if (PalExt == (char **) 0) {
		if ((PalExt = (char **) malloc(sizeof(char *))) == (char **) 0) {
	...
	for (NumExt = 0; PalExt[NumExt] != (char) 0; NumExt++);
	...
	while (PosAct < LongTxt) {
		...
		for (Ext = 0; Ext < NumExt; Ext++) {
			if ((Long = strlen(PalExt[Ext])) != GrpAct.Long) continue;
		...
		for (Ext = 0; DicExc && DicExc[Ext]; Ext++) {
			if ((Long = strlen(DicExc[Ext][1])) != GrpAct.Long) continue;
		...
		for (Ext = 0; DicTrn && DicTrn[Ext]; Ext++) {
			if ((Long = strlen(DicTrn[Ext][0])) != GrpAct.Long) continue;
		...
		if (((GrpAct.Tipo & PALABRA) && EsPalExt(GrpAct)) || (GrpAct.Tipo & DESCONOCIDO)) {
			...
			if ((PalExt[NumExt-1] = (char *) calloc(GrpAct.Long+1, sizeof(char))) == (char *) 0) {
				fprintf(stderr, "Error al ubicar memoria para PalExt\n");
				return (char **) 0;
			}
			strncpy(PalExt[NumExt-1], GrpAct.Cont, GrpAct.Long);
			PalExt[NumExt] = (char *) 0;
		}
...

{
	...
	while (Pos < (int) GrpOrt.Long) {
		...
		while ((Chr = IndexChr(GrpOrt.Cont+Pos, Vocales)) >= 0) {
			if (!strcmp(Vocales[Chr], "y") && NumVocs > 0) break;
