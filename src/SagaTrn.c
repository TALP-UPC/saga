    ...
    while (Pos < (int) GrpAct.Long)
    {
        ...
        while ((Chr = IndexChr(SilAct + Ind, engine->Letras)) >= 0)
        {
            ...
            if (Ind == (int) strlen(SilAct))
            {
                ...
                if (Chr >= 0)
                    strcpy(ChrSig, engine->Letras[Chr]);
            }
            else
            {
            ...
            else if (strchr(ChrAct, 'u') != (char *) 0)
            {
                ...
                else if (!strcmp(ChrAct, "u") && IniSil && IniPal && FinSil
                         && FinPal)
                {
            ...
            else if (!strcmp(ChrAct, "y"))
            {
                ...
                else if ((IniSil && IniPal
                          && IndexChr(ChrSig, engine->Vocales) >= 0)
                         || (IniSil
                             && IndexChr(ChrAnt, engine->ConsTxt) >= 0))
                {
                    strcat(TrnPal, "jj");
                }
                ...
                else
                {
                    ...
                    EsVocal = 1;
                     ...
                     && (!strcmp(ChrAct, "p") || !strcmp(ChrAct, "b")
                         ...
                         || !strcmp(ChrAct, "g") || !strcmp(ChrAct, "j")))
            {
                strcat(TrnPal, "G");
            }
             ...
             */
            else if (!strcmp(ChrAct, "s") || !strcmp(ChrAct, "x")
                     || !strcmp(ChrAct, "~c"))
            {
                SigueEse = 0;
...
*/
                ...
                else if ((engine->ClaveModif & ESE_ASP_CON) && FinSil)
                {
                    if (FinPal && FinSil &&
                        (IndexChr(ChrSig, engine->Vocales) >= 0
                         || IndexChr(ChrSig, engine->Letras) < 0
                ...
                else if (!strcmp(ChrSig, "y")
                         && (!FinPal || GrpSig.Long != 1))
                {
                    strcat(TrnPal, "z");
                }
            ...
            else if (!strcmp(ChrAct, "z"))
            {
                if (engine->ClaveModif & SESEO)
                {
                    ...
                    {
                        Chr =
                            IndexChr(SilSig + strlen(ChrSig), engine->Letras);
                        if (Chr >= 0 && strchr(engine->Letras[Chr], 'e'))
                        {
                            strcat(TrnPal, "z");
                        }
            ...
            {
                char *Acento;
                if ((Acento = strchr(GrpAct.Cont, '\''))
                    && Acento - GrpAct.Cont < Pos)
                {
                    strcat(TrnPal, "_");
                }
            }
                ...
                && TrnPal[strlen(TrnPal) - 1] != ':')
            {
                ...
                else if (EraVocal && strchr("jw", TrnPal[strlen(TrnPal) - 1])
                         && strcmp("jj", TrnPal + strlen(TrnPal) - 2))
                {
                    strcat(TrnPal, ":");
                }
                else if (FinSil
                ...
                && IndexChr(ChrSig, Nasales) >= 0)
            {
                if (IndexChr(ChrAnt, Nasales) >= 0
                    || IndexChr(ChrAnt, engine->Letras) < 0)
                {
                    strcat(TrnPal, "~");
                }
            }
...
char *TrnFonFnm(char *TrnFon, int ConSil, char **Fonemas)
{
     ...
     */
    if ((TrnFnm =
         (char *) calloc((size_t) (20 * LongFon + 1),
                         sizeof(char))) == (char *) 0)
    {
    ...
    while (PosAct < LongFon)
    {
        ...
        {
            fprintf(stderr,
                    "Error al localizar el siguiente grupo de SilOrt\n");
            free(TrnFnm);
            return NULL;
        }
...
char *TrnFonFnmPal(char *TrnFon, int ConSil, char **Fonemas)
{
    ...
    while (PosAct < LongFon)
    {
        ...
        if (PosAct > 0
            && ((GrpAct.Tipo & ESP_FON) == 0
                && (ConSil || (GrpAct.Tipo & (PUNT_FON | SIL_FON)) == 0)))
        {
            if (Espacio == 0)
                strcat(TrnFnm, " ");
            strcat(TrnFnm, "/");
...
char *TrnFonSefo(char *TrnFon,
                 int ConSil,
                 const char *StrIniPal, const char *StrFinPal, char **Fonemas)
{
