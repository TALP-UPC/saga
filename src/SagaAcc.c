...
char *AcenSilOrt(char *SilOrt, char ***DicTrnPal, SagaEngine *engine)
{
     ...
     */
    if ((AccSil =
         (char *) calloc((size_t) (20 * LongTxt + 1),
                         sizeof(char))) == (char *) 0)
    {
...
char *AcenPalSil(GRP_ORT GrpOrt, SagaEngine *engine)
{
    ...
    if ((Pos > 0) && !strcmp(PalSil + Pos, "-men-te")
        && ((int) strcspn(PalSil, "-") < Pos))
    {
        Mente = 1;
        PalSil[Pos] = '\0';
        Long = Pos;
    ...
    else
    {
         ...
         */
        for (PosUltSil = strlen(PalSil); PalSil[PosUltSil - 1] != '-';
             PosUltSil--);
        for (PosPenSil = PosUltSil - 1;
             PosPenSil > 0 && PalSil[PosPenSil - 1] != '-'; PosPenSil--);

        UltSilFilled = 0;
        if (safe_strcat
            (&UltSil, &PalSil[PosUltSil], &UltSilSize, &UltSilFilled) < 0)
        {
            free(AcenPal);
            free(PalSil);
            free(UltSil);
        ...
        PenSilFilled = 0;
        if (safe_strcat
            (&PenSil, &PalSil[PosPenSil], &PenSilSize, &PenSilFilled) < 0)
        {
            free(AcenPal);
            free(PalSil);
            free(UltSil);
        ...
        if (((LongPalSil > 0)
             && strchr("aeiouns", PalSil[LongPalSil - 1]) != (char *) 0)
            || ((LongPalSil > 1) && PalSil[LongPalSil - 1] == 'y'
                && !strchr("aeiou", PalSil[LongPalSil - 2])))
        {
...
int AcenSil(char *Sil, SagaEngine *engine)
{
    ...
    while ((Chr = IndexChr(Sil + Pos, engine->ConsTxt)) >= 0)
    {
        ...
        if (strcmp(engine->ConsTxt[Chr], "g") == 0)
        {
            ...
            if (strcmp(engine->Letras[Chr1], "u") == 0 && Chr2 >= 0
