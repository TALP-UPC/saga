...
int AplDicPal(char **Frase, int PosAct, int LongPal, char ***DicExc);
...
char ***CargDicExc(const char *FicDic)
{
    ...
    for (Fic = 1; Fic < NumFic; Fic++)
    {
        ...
        for (Pal = 0; OtroDic[Pal] != NULL; Pal++)
        {
            if ((tmpDicExc =
                 realloc(DicExc,
                         (TamDic + Pal + 2) * sizeof(char **))) == NULL)
            {
                fprintf(stderr,
                        "Error al ubicar memoria para el diccionario (%s)\n",
                        strerror(errno));
                BorraDicExc(DicExc);
                BorraDicExc(OtroDic);
                LiberaMatStr(LisFic);
...
char ***ReadDicExc(char *FicDicExc)
{
    ...
    {
        if ((tmpDicExc =
             realloc(DicExc, (Long + 2) * sizeof(char **))) == NULL)
        {
            fprintf(stderr,
                    "Error al reubicar memoria para el diccionario\n");
            fclose(FpDicExc);
            BorraDicExc(DicExc);
            return NULL;
        }
                    ...
                    for (Ind = 0; DicSust[Ind] != (char **) 0; Ind++)
                    {
                        if (!strcmp(Fonemas[Chr], DicSust[Ind][0]))
                        {
                            ...
                            if (LongOut > LongIn)
                            {
                                ...
                                {
                                    (*TrnFon)[Pos2] =
                                        (*TrnFon)[Pos2 + LongIn - LongOut];
                                }
                            }
                            else if (LongOut < LongIn)
                            {
                                for (Pos2 = PosAct + 1; Pos2 <= LongFon;
                                     Pos2++)
                                {
                                    (*TrnFon)[Pos2] =
                                        (*TrnFon)[Pos2 + LongIn - LongOut];
                            ...
                            for (Pos2 = PosAct; Pos2 < PosAct + LongOut;
                                 Pos2++)
                            {
                                (*TrnFon)[Pos2] =
                                    DicSust[Ind][1][Pos2 - PosAct];
                            }
...
int AplDicPal(char **Frase, int _PosAct, int LongPal, char ***DicExc)
{
    char *Mascara = NULL;
    ...
    {
        if (strcmp(DicExc[Ind][0], Palabra) != 0)
            continue;
        ...
        {
            if ((*Frase =
                 (char *) realloc((void *) *Frase,
                                  (LongFrs + 1) * sizeof(char))) ==
                (char *) 0)
            {
                fprintf(stderr, "Error de memoria\n");
                free(Mascara);
    ...
    for (Ind = 0; DicExc[Ind] != (char **) 0; Ind++)
    {
        ...
        MascaraFilled = 0;
        if (safe_strcat
            (&Mascara, DicExc[Ind][0], &MascaraSize, &MascaraFilled) < 0)
        {
            free(Mascara);
            free(_Palabra);
            return -1;
        ...
        if (IniCom)
        {
            if (strstr(Palabra, PalDic) == (char *) 0)
                continue;
            IniPal = Palabra;
            PosAct = _PosAct;
        ...
        else if (strncmp(Palabra, PalDic, LongExc) != 0)
            continue;

        if (FinCom)
        {
            if (strncmp(Palabra, PalDic, LongExc) != 0)
                continue;
            LongPal = LongExc;
        }
        else
        {
            if (strncmp(Palabra + LongPal - LongExc, PalDic, LongExc) != 0)
                continue;
        }
